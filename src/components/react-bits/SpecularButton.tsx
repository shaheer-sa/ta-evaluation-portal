...
const SpecularButton = ({
  ...
  useEffect(() => {
    ...
    // A dead WebGL context must not take the login page down with it.
    ...
    const resize = () => {
      ...
      // border instead of drifting a pixel from offsetWidth rounding.
  ...
  const classes = ['rb-spec', `rb-spec--${size}`];
  if (block) classes.push('rb-spec--block');
  ...
  return (
    ...
    >
      <span ref={fxRef} aria-hidden="true" className="rb-spec__fx" />
      <span className="rb-spec__label">{children}</span>
    </button>
  );
};
