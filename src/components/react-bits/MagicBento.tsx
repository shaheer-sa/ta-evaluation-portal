...
export interface MagicBentoProps {
  ...
  onCardClick?: (card: BentoCard, index: number) => void;
  className?: string;
}
...
const ParticleCard: React.FC<{
  ...
  }, [animateParticles, clearAllParticles, disableAnimations, enableTilt, enableMagnetism, clickEffect, onActivate]);
...
const GlobalSpotlight: React.FC<{
  ...
  useEffect(() => {
    ...
    spotlight.className = 'mc-spotlight';
    spotlight.style.setProperty('--mc-glow', glowColor);
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      const section = gridRef.current.closest('.mc-section');
      ...
      const cards = gridRef.current.querySelectorAll('.mc-card');
    ...
    const handleMouseLeave = () => {
      gridRef.current?.querySelectorAll('.mc-card').forEach(card => {
...
const MagicBento: React.FC<MagicBentoProps> = ({
  ...
  const cardClass = enableBorderGlow ? 'mc-card mc-card--glow' : 'mc-card';

  return (
    <section
      className={className ? `mc-section ${className}` : 'mc-section'}
      style={{ '--mc-glow': glowColor } as CSSProperties}
    >
      {enableSpotlight && (
        <GlobalSpotlight
          gridRef={gridRef}
      ...
      <div className="mc-grid" ref={gridRef}>
        {cards.map((card, index) => {
          const style = card.color ? ({ background: card.color } as CSSProperties) : undefined;

          const content = (
            <>
              <div className="mc-card__head">
                <span className="mc-card__label">{card.label}</span>
                {card.delta && (
                  <span className="mc-card__delta" data-tone={card.deltaTone ?? 'flat'}>
                    {card.delta}
                  </span>
                )}
              </div>
              <div className="mc-card__body">
                {card.value !== undefined && <p className="mc-card__value">{card.value}</p>}
                <h3 className={textAutoHide ? 'mc-card__title mc-clamp-1' : 'mc-card__title'}>{card.title}</h3>
                {card.description && (
                  <p className={textAutoHide ? 'mc-card__desc mc-clamp-2' : 'mc-card__desc'}>{card.description}</p>
                )}
              </div>
            </>
          );
